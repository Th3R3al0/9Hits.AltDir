#!/bin/bash
DEFAULT_DOWNLOAD="https://www.dropbox.com/s/usku8rcyysk9664/9hits-linux-x64-2.5.2.tar.bz2"
_9HITSUSER="_9hits"
...
function get_app() {
	...
	wget -O "/tmp/get-app.sh" https://9hitste.github.io/install/5.0.0/get-app.sh && chmod +x "/tmp/get-app.sh"
	runuser -l $_9HITSUSER -c "/bin/bash /tmp/get-app.sh $(printf "%q" "$ARGS")"
	rm -f "/tmp/get-app.sh"
}

function check_custom_dir_and_swap() {
	for i in $ARGS; do
	  case $i in
		--install-dir=*)
			if [ ! -d "${i#*=}" ];then
		...
		--create-swap=*)
			CREATE_SWAP="${i#*=}"
			swapoff "/tmp/9hits_swap" && rm -f "/tmp/9hits_swap"
			fallocate -l "$CREATE_SWAP" "/tmp/9hits_swap" && chmod 600 "/tmp/9hits_swap" && mkswap "/tmp/9hits_swap" && swapon "/tmp/9hits_swap"
		  shift # past argument=value
		  ;;
		*)
		  ;;
